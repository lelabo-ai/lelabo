...
def _load_modules(monkeypatch: pytest.MonkeyPatch):
    ...
    fa_mod = importlib.import_module("lelabo.update_rules.builtins.fa")
    return mlp_mod, conv_mod, task_mod, fa_mod
...
def test_fa_updates_mlp_hidden_and_head(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fa_updates_conv_and_head(monkeypatch: pytest.MonkeyPatch) -> None:
    torch = pytest.importorskip("torch")
    _mlp_mod, conv_mod, task_mod, fa_mod = _load_modules(monkeypatch)
