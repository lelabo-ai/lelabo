...
class KP(UpdateRule):
    ...
    NEW BEHAVIOR (what you asked):
      - When train_step() is called while phase == "pretrain":
          it will internally loop trunk-pretrain until the stagnation criterion triggers,
          THEN run exactly ONE finetune (BP) step on the same batch and return.
      - Subsequent calls (phase == "finetune") just do one BP step as usual.
    ...
    def __init__(
        ...
        # --- trunk pretrain schedule (supervised only) ---
        trunk_pretrain=True,
        ...
        # --- backprop after pretrain (supervised only) ---
    ...
    def train_step(self, model, task, batch, device):
        ...
        if isinstance(batch, Mapping):
            ...
            # ---- NEW: loop trunk-pretrain internally until stagnation, then do 1 finetune step, then return.
            if self._phase == "pretrain":
                pretrain_inner_steps = 0
                last_pretrain_stats: Dict[str, float] = {"loss": 0.0}
                ...
                while self._phase == "pretrain":
                    ...
                    # stats for the last inner pretrain iteration
                    ...
                    last_pretrain_stats = stats
                    pretrain_inner_steps += 1
                    self.global_step += 1
                ...
                # If we switched, do exactly ONE finetune step on this same batch and return.
                if self._phase == "finetune" and self.bp_after_pretrain:
                    if y_true is None:
                        raise ValueError(
                            "SoftHebb finetune (HF dict): expected 'labels' in batch for supervised BP."
                        )

                    # optionally capture x/u during BP forward (only needed if continuing SoftHebb)
                    layer_cache: List[Dict[str, Any]] = []
                    hooks: List[Any] = []
                    ...
                    fin_stats = {"loss": float(loss.item())}
                    fin_stats.update(extra)

                    if torch.is_tensor(y_true) and y_true.dtype in (torch.int64, torch.int32, torch.int16):
                        preds = logits.argmax(dim=-1)
                        fin_stats["acc"] = float((preds == y_true).float().mean().item())

                    fin_stats["phase"] = 1.0
                    fin_stats["pretrain_inner_steps"] = float(pretrain_inner_steps)
                    self.global_step += 1
                    return fin_stats

                # If no BP-after-pretrain, just return the last pretrain stats.
                last_pretrain_stats["pretrain_inner_steps"] = float(pretrain_inner_steps)
                return last_pretrain_stats

            # ---- Supervised finetune: one BP step
            if self._phase == "finetune" and self.bp_after_pretrain:
                layer_cache: List[Dict[str, Any]] = []
                hooks: List[Any] = []
                if self.continue_softhebb_after_pretrain:
        ...
        if self._phase == "pretrain":
            ...
            while self._phase == "pretrain":
                ...
                if logits is None or not torch.is_tensor(logits):
                    raise ValueError(
                        "SoftHebb supervised pretrain: model output must be a logits tensor (or dict with 'logits')."
                    )
            ...
            if self._phase == "finetune" and self.bp_after_pretrain:
                ...
                if logits is None or not torch.is_tensor(logits):
                    raise ValueError(
                        "SoftHebb supervised finetune: model output must be a logits tensor (or dict with 'logits')."
                    )
                ...
                if y.dtype in (torch.int64, torch.int32, torch.int16):
                    preds = logits.argmax(dim=-1)
                    fin_stats["acc"] = float((preds == y).float().mean().item())
        ...
        if self._phase == "finetune" and self.bp_after_pretrain:
            ...
            if y.dtype in (torch.int64, torch.int32, torch.int16):
                preds = logits.argmax(dim=-1)
                stats["acc"] = float((preds == y).float().mean().item())
            self.global_step += 1
            return stats
